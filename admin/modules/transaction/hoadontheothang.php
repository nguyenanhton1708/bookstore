<?php 
	...
	$sql = "SELECT transaction.*, user.name as nameuser, user.phone as phoneuser FROM transaction LEFT JOIN user ON user.id = transaction.users_id where MONTH(transaction.updated_at)=MONTH(NOW()) and transaction.status=1 ORDER BY ID DESC ";
